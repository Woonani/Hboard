...
document.addEventListener("DOMContentLoaded", () => {
  ...
  signInForm.id.addEventListener("blur", () => {
    ...
    if (
      signInForm.id.value.trim() != "" &&
      !pattern.test(signInForm.id.value)
    ) {
      alert(
        "유효하지 않은 형식입니다. id는 영문자로 시작하는 4~15자의 영문소문자, 숫자로 작성해주세요."
      );
      signInForm.id.value = "";
      signInForm.id.focus();
    }
  ...
  signInForm.pw1.addEventListener("blur", () => {
    ...
    if (
      signInForm.pw1.value.trim() != "" &&
      !pattern.test(signInForm.pw1.value)
    ) {
      alert(
        "유효하지 않은 형식입니다. 비밀번호는 8~15자의 영문자/숫자 혼합으로 작성해주세요."
      );
  ...
  signInForm.pw2.addEventListener("blur", () => {
    if (
      signInForm.pw2.value.trim() != "" &&
      signInForm.pw1.value != signInForm.pw2.value
    ) {
  ...
  signInForm.addEventListener("submit", function (event) {
    ...
    // input 세팅
    let emailAddr =
      signInForm.email1.value.trim() + "@" + signInForm.email2.value.trim();
    ...
    let data = new URLSearchParams({
      ...
      email: emailAddr,
      phone:
        signInForm.phone1.value +
        "-" +
        signInForm.phone2.value +
        "-" +
        signInForm.phone3.value,
      tel:
        signInForm.tel1.value +
        "-" +
        signInForm.tel2.value +
        "-" +
        signInForm.tel3.value,
